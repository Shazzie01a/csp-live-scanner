...
export default function CSPLiveScanner() {
  ...
  const [news,      setNews]      = useState({})  // { TICKER: [{title, link, age}] }
  const [newsLoading, setNewsLoading] = useState({}) // { TICKER: true/false }

  // Fetch news for a single ticker on demand
  const fetchNews = async (ticker) => {
    if (news[ticker] || newsLoading[ticker]) return // already loaded or loading
    setNewsLoading(n => ({...n, [ticker]: true}))
    try {
      ...
      setNews(n => ({...n, [ticker]: data.articles || []}))
    } catch(e) {
      setNews(n => ({...n, [ticker]: []}))
    }
    setNewsLoading(n => ({...n, [ticker]: false}))
  }
          ...
          {rows.map(r => {
            ...
            return (
              <div key={r.ticker} className={`card${isOpen?' open':''}`}>
                {/* Ticker header */}
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10,cursor:'pointer'}} onClick={()=>{
                  const newExpanded = isOpen ? null : r.ticker
                  setExpanded(newExpanded)
                  if (newExpanded) fetchNews(newExpanded)
                }}>
                ...
                {isOpen && (
                  <div style={{padding:'12px 14px',background:'#0a0a18',borderTop:'1px solid #141428',fontSize:12,color:'#cce',lineHeight:1.8,borderRadius:'0 0 10px 10px',marginTop:10}}>
                    ...
                    <div style={{marginTop:10,paddingTop:10,borderTop:'1px solid #1a1a2e'}}>
                      <div style={{fontSize:9,fontWeight:700,letterSpacing:'.1em',color:'#fff',marginBottom:8}}>📰 LATEST NEWS</div>
                      {newsLoading[r.ticker] && (
                        <div style={{color:'#444',fontSize:11}}>Loading news...</div>
                      )}
                      {!newsLoading[r.ticker] && news[r.ticker]?.length === 0 && (
                        <div style={{color:'#444',fontSize:11}}>No recent news found.</div>
                      )}
                      {!newsLoading[r.ticker] && news[r.ticker]?.map((article, i) => (
                        <div key={i} style={{marginBottom:8,paddingBottom:8,borderBottom:i<news[r.ticker].length-1?'1px solid #111':'none'}}>
                          <div style={{display:'flex',alignItems:'flex-start',gap:8}}>
                            <span style={{
                              fontSize:9,fontWeight:700,padding:'2px 6px',borderRadius:3,whiteSpace:'nowrap',marginTop:1,
                              background:article.ageHours<=2?'rgba(255,77,109,.2)':article.ageHours<=6?'rgba(255,170,0,.2)':'rgba(255,255,255,.06)',
                              color:article.ageHours<=2?'#ff4d6d':article.ageHours<=6?'#ffaa00':'#666',
                              border:`1px solid ${article.ageHours<=2?'rgba(255,77,109,.3)':article.ageHours<=6?'rgba(255,170,0,.3)':'#222'}`
                            }}>
                              {article.ageHours<=1?'JUST NOW':article.ageHours<=2?`${Math.round(article.ageHours*60)}m ago`:article.ageHours<=24?`${Math.round(article.ageHours)}h ago`:`${Math.round(article.ageHours/24)}d ago`}
                            </span>
                            <a href={article.link} target="_blank" rel="noopener noreferrer"
                              style={{color:'#cce',fontSize:11,lineHeight:1.5,textDecoration:'none',flex:1}}
                              onClick={e=>e.stopPropagation()}>
                              {article.title}
                              <span style={{color:'#7ec8e3',marginLeft:6,fontSize:10}}>↗</span>
                            </a>
                          </div>
                        </div>
                      ))}
