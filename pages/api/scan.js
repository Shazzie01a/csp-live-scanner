...
export default async function handler(req, res) {
  ...
  try {
    // Fetch all quotes in parallel with timeout
    const withTimeout = (promise, ms) => Promise.race([
      promise,
      new Promise(resolve => setTimeout(() => resolve(null), ms))
    ])

    // Batch quotes to avoid overwhelming Yahoo
    const batchSize = 20
    const quoteResults = []
    for (let i = 0; i < ALL_TICKERS.length; i += batchSize) {
      const batch = ALL_TICKERS.slice(i, i + batchSize)
      const batchResults = await Promise.all(
        batch.map(t => withTimeout(getYahooQuote(t), 4000))
      )
      quoteResults.push(...batchResults)
    }

    // Build results using price-based estimates (fast, no options chain needed)
    // Options chain fetch is too slow for 65+ tickers on free Vercel
    const results = ALL_TICKERS.map((ticker, i) => {
      const cfg   = getConfig(ticker)
      const q     = quoteResults[i] || {}
      const price = q.price || 0

      // Price-based estimates (scales with live price)
      const strike     = price > 0 ? Math.round(price * 0.92) : 0
      const premium    = price > 0 ? parseFloat((price * 0.027).toFixed(2)) : 0

      // Use IV estimate based on category/risk
      const iv = cfg.risk === 'high' ? 75 : cfg.risk === 'low' ? 25 : 45
      const ivRank = cfg.risk === 'high' ? 70 : cfg.risk === 'low' ? 25 : 50
      const dte = 30
      ...
      return {
        ...
        play_why:   `Live price $${price} · Est. strike $${strike} · Est. premium $${premium} · ${cfg.note}`,
        strike_pct: 0.92,
        premium_pct: 0.027,
        isLive:     true,
        dataSource: 'Yahoo Finance (live price · est. premium)',
        lastUpdated: new Date().toISOString(),
      }
    }).filter(r => r.price > 0) // Only show tickers with valid prices
